lametayel.controller('activitiesCtrl', ['$scope', '$stateParams', 'server', '$rootScope', '$state', '$filter', function ($scope, $stateParams, server, $rootScope, $state, $filter) {
    ...
    $scope.activityClick = function (item) {
        //toggle the item.picked
        if (item.picked == true || item.picked == false) {
        ...
        //}
        $scope.activityChosen = $filter('filter')($scope.activArray, { picked: true });
        console.log($scope.activityChosen)
    }
    ...
    $scope.createList = function () {
        ...
        if (!$rootScope.addActStatus) {
            if (!($scope.activityChosen.length > 0 && $scope.weatherChosen.length > 0)) {
                alert('צריך לבחור לפחות פריט אחד מכול רשימה');
                return;
            }
            var weatherChosen = "";
